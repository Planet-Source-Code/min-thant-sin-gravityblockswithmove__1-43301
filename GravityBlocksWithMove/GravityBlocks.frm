...
Begin VB.Form frmPuzzle 
   ...
   Begin VB.PictureBox picFade 
      ...
      Height          =   4500
      Left            =   5475
      Picture         =   "GravityBlocks.frx":B00A
      ScaleHeight     =   300
      ScaleMode       =   3  'Pixel
      ScaleWidth      =   250
      TabIndex        =   3
   ...
   Begin VB.PictureBox picBlocks 
      ...
      Height          =   750
      Left            =   75
      Picture         =   "GravityBlocks.frx":4218C
      ScaleHeight     =   50
      ScaleMode       =   3  'Pixel
      ScaleWidth      =   300
      TabIndex        =   2
...
Private BlockToMove As Integer


Private Sub Form_Load()
      boolProcessing = False
      boolPlaySound = True
      boolMovableBlocks = True
      
      'Init block width & height variables
      BlockWidth = frmPuzzle.picBlank.ScaleWidth
      BlockHeight = frmPuzzle.picBlank.ScaleHeight
      
      Call InitPuzzle(4, 15, 10)
End Sub

Private Sub Form_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
      If GameOver Then Exit Sub
      Call ReDisplayHintedBlocks
      picBoard.Refresh
End Sub

Sub ReDisplayHintedBlocks()
      Dim I As Integer
      Dim BlockIndex As Integer
      
      For I = 1 To colHintBlocks.Count
            BlockIndex = colHintBlocks(I)
            If Blocks(BlockIndex).Exists Then
                  BlitImage BlockIndex
                  Blocks(BlockIndex).ShowingHint = False
            End If
      Next I
End Sub
...
Private Sub picBoard_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
      ...
      If Not boolMovableBlocks Then Exit Sub
      'Blocks are being moved
      If boolProcessing Then Exit Sub
...
Private Sub picBoard_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
      ...
      '/////////////////////////////////////////////////////////////
      Dim HintBlock As Integer
      Dim BlockIndex As Integer
      Dim I As Integer
      
      If Button <> vbLeftButton Then
            If Button <> vbRightButton And Button <> vbMiddleButton Then
                  HintBlock = GetBlockUnderMouse(X, Y)
                  
                  If Blocks(HintBlock).ShowingHint Then Exit Sub
                  Call ReDisplayHintedBlocks
                  
                  Set colHintBlocks = Nothing
                  Set colHintBlocks = New Collection
                  
                  If Blocks(HintBlock).Exists Then
                        FindSameBlocks HintBlock, colHintBlocks
                  End If
                  
                  For I = 1 To colHintBlocks.Count
                        BlockIndex = colHintBlocks(I)
                        Blocks(BlockIndex).HasBeenFound = False
                        Blocks(BlockIndex).ShowingHint = True
                        BlitHint BlockIndex
                  Next I
            End If
            picBoard.Refresh
      ...
      If Not boolMovableBlocks Then Exit Sub
      If BlockToMove = -1 Then Exit Sub
      ...
      If Blocks(Index).Exists Then Exit Sub
      
      boolDragging = True
      
      If Blocks(Index).YCoord = Blocks(BlockToMove).YCoord Then
            If Abs(Blocks(Index).XCoord - Blocks(BlockToMove).XCoord) = 1 Then
                  If Blocks(Index).XCoord > Blocks(BlockToMove).XCoord Then
                        MoveRight BlockToMove
                  Else
                        MoveLeft BlockToMove
                  End If
            End If
      End If
...
Function MovableBlock(ByVal Index As Integer) As Boolean
      Dim AboveBlock As Integer
      Dim LeftBlock As Integer, RightBlock As Integer
      ...
      If AboveBlock <> -1 Then
            If Blocks(AboveBlock).Exists Then Exit Function
      End If
      
      If LeftBlock <> -1 Then
            If Blocks(LeftBlock).Exists = False Then
                  MovableBlock = True
                  Exit Function
            End If
      End If
      ...
      If RightBlock <> -1 Then
            If Blocks(RightBlock).Exists = False Then
                  MovableBlock = True
                  Exit Function
            End If
      End If
...
Private Sub picBoard_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
      ...
      Call FindSameBlocks(Blocks(Index).ID, colSameBlocks)
      ...
      For I = 1 To colSameBlocks.Count
            ...
            Blocks(Index).ShowingHint = False
